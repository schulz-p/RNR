...
Implementation from the GitHub repository by Bob de Vos (AmsterdamUMC) https://github.com/BDdeVos/TorchIR 
with one small adaptation (see row 47). """
...
class NCC(_Loss):
    ...
    def metric(self, fixed: Tensor, warped: Tensor) -> Tensor:
        return 1-self.ncc(fixed, warped)    # adaptation of original code: '-self.ncc(fixed, warped).mean()'
                                            # reasons for adaptations: definition of NCC as in literature, mean() not required
